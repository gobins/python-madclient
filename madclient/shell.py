"""
Command-line interface to the MAD Service API.
"""
...
from madclient import client
import madclient.extension
from madclient.openstack.common import cliutils
from madclient.openstack.common.apiclient import auth
from madclient.openstack.common.apiclient import exceptions as exc


DEFAULT_OS_SHARE_API_VERSION = "1"
DEFAULT_MAD_ENDPOINT_TYPE = 'publicURL'
DEFAULT_MAD_SERVICE_TYPE = 'active_directory'
...
class MadClientArgumentParser(argparse.ArgumentParser):

    def __init__(self, *args, **kwargs):
        super(MadClientArgumentParser, self).__init__(*args, **kwargs)
    ...
    def _get_option_tuples(self, option_string):
        ...
        """
        option_tuples = super(
            MadClientArgumentParser, self)._get_option_tuples(option_string)
...
class MadKeyring(keyring.backends.file.EncryptedKeyring):
    def delete_password(self, keyring_keys):
        ...
        for key, value in six.iteritems(keyring_keys):
            super(MadKeyring, self).delete_password(key, value)
...
class SecretsHelper(object):
    ...
    def check_cached_password(self):
        ...
        if self.args.os_cache:
            ...
            cached_token = self.keyring.get_password('madclient_auth',
                                                     self._make_key())
    ...
    def management_url(self):
        ...
        if not self.args.os_cache:
            return None
        management_url = None
        block = self.keyring.get_password('madclient_auth',
                                          self._make_key())
        if block:
...
class OpenStackMadShell(object):

    def get_base_parser(self):
        parser = MadClientArgumentParser(
            prog='mad',
            description=__doc__.strip(),
            epilog='See "mad help COMMAND" '
        ...
        parser.add_argument('--version',
                            action='version',
                            version=madclient.__version__)

        parser.add_argument('--debug',
                            action='store_true',
                            default=cliutils.env('madclient_DEBUG',
                                                 default=False),
        ...
        parser.add_argument('--service-name',
                            ...
                            default=cliutils.env('MAD_SERVICE_NAME'),
                            help='Defaults to env[MAD_SERVICE_NAME].')
        parser.add_argument('--service_name',
                            help=argparse.SUPPRESS)

        parser.add_argument('--mad-service-name',
                            metavar='<mad-service-name>',
                            default=cliutils.env('MAD_service_name'),
                            help='Defaults to env[MAD_service_name].')
        ...
        parser.add_argument('--endpoint-type',
                            ...
                            default=cliutils.env(
                                'MAD_ENDPOINT_TYPE',
                                default=DEFAULT_MAD_ENDPOINT_TYPE),
                            help='Defaults to env[MAD_ENDPOINT_TYPE] or '
                            + DEFAULT_MAD_ENDPOINT_TYPE + '.')
        parser.add_argument('--endpoint_type',
                            help=argparse.SUPPRESS)

        parser.add_argument('--os-mad-api-version',
                            metavar='<compute-api-ver>',
                            default=cliutils.env(
                                'OS_MAD_API_VERSION',
                                default=DEFAULT_OS_MAD_API_VERSION),
                            help='Accepts 1 or 2, defaults '
                                 'to env[OS_MAD_API_VERSION].')
        parser.add_argument('--os_mad_api_version',
                            help=argparse.SUPPRESS)
        ...
        parser.add_argument('--insecure',
                            default=cliutils.env('madclient_INSECURE',
                                                 default=False),
                            action='store_true',
                            help=argparse.SUPPRESS)
    ...
    def _discover_extensions(self, version):
        ...
        for name, module in itertools.chain(
            ...
            extension = madclient.extension.Extension(name, module)
    ...
    def main(self, argv):
        ...
        if not endpoint_type:
            endpoint_type = DEFAULT_MAD_ENDPOINT_TYPE

        if not service_type:
            service_type = DEFAULT_MAD_SERVICE_TYPE
        ...
        self.cs = client.Client(options.os_mad_api_version, os_username,
                                ...
                                mad_service_name=mad_service_name,
        ...
        if not cliutils.isunauthenticated(args.func):
            ...
            if (helper.tenant_id and helper.auth_token and
                ...
                try:
                    ...
                    raise exc.CommandError("Invalid OpenStack Mad "
                                           "credentials.")
...
def main():
    try:
        if sys.version_info >= (3, 0):
            OpenStackMadShell().main(sys.argv[1:])
        else:
            OpenStackMadShell().main(
                map(encodeutils.safe_decode, sys.argv[1:]))
    except KeyboardInterrupt:
        print("... terminating mad client", file=sys.stderr)
        sys.exit(130)
